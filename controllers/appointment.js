...
exports.getAppointments = async (req,res,next)=>{
    ...
    if(req.user.role != 'admin'){
        query=Appointment.find({user:req.user.id}).populate({//$Lookup
            path:'hospital',
            select:'name province tel'
        });
    }
    else{ //If you are an admin, you can see it all!
...
exports.getAppointment = async (req,res,next)=>{
    try{
        const appointment = await Appointment.findById(req.params.id).populate({//$Lookup
            path:'hospital',
            select:'name province tel'
        });
...
exports.addAppointment = async (req,res,next)=>{
    try{
        ...
        if(!hospital) return res.status(404).json({success:false,message:`No hospital with the id of ${req.params.hospitalId}`});
...
exports.updateAppointment = async (req,res,next)=>{
    try{
        ...
        console.log(err);
        return res.status(500).json({success:false,message:"Cannot update to Appointment"});
    }
};
